...
class Users_model extends CI_Model {
	...
	function user_login($data){
		$sql = "SELECT * FROM customers WHERE email ='".$data['email']."' AND password ='".$data['password']."'";
		///echo $sql;exit;
        return $this->db->query($sql)->row_array(); 
	}
	function getcustdetails($cust_id){
		$sql = "SELECT * FROM customers WHERE user_id ='".$cust_id."'";
		///echo $sql;exit;
        return $this->db->query($sql)->row_array(); 
	}
	function listquotations($user_id)
	{
		$this->db->select('*')->from('quotations');
		$this->db->where('user_id', $user_id);
		$this->db->order_by('quotations.quote_id DESC');
	...
	function update_password($reset_pass){
		$sql1="UPDATE customers SET password ='".md5($reset_pass['conpassword'])."'WHERE user_id = '".$reset_pass['userid']."' AND email='".$reset_pass['email']."'";
		return $this->db->query($sql1);
	}
	...
	function updatecompanyprofile($data,$where)
	{
