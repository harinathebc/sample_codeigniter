...
class AuthorizeNetAIM extends AuthorizeNetRequest
{
    ...
    const LIVE_URL = 'https://secure.authorize.net/gateway/transact.dll';//'https://developer.authorize.net/tools/paramdump/index.php';//
     ...
     */
    public function authorizeAndCapture($amount = false, $card_num = false, $exp_date = false,$card_code='',$state = '',$zip_code= '',$country = '')
    {
		...
		$fields = array();
		//if($fst_name != ''){ $this->first_name = $fst_name; }
		//if($last_name != ''){ $this->last_name = $last_name; }
    	//if($city != ''){ $this->city = $city; }
		if($card_code != ''){ $this->card_code = $card_code; }
		if($state != ''){ $this->state = $state; }
    	if($zip_code != ''){ $this->zip = $zip_code; }
